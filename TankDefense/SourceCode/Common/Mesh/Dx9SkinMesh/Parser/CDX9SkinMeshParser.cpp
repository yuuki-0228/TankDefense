...
D3DXVECTOR3 D3DXPARSER::GetTangent(MYMESHCONTAINER* pContainer, DWORD Index)
{
	LPDIRECT3DVERTEXBUFFER9 pVB = nullptr;
	pContainer->MeshData.pMesh->GetVertexBuffer(&pVB);
	DWORD Stride = pContainer->MeshData.pMesh->GetNumBytesPerVertex();
	BYTE *pVertices = nullptr;
	D3DXVECTOR3* pUV = nullptr;
	...
	if (SUCCEEDED(pVB->Lock(0, 0, (VOID**)&pVertices, 0)))
	{
		...
		pVertices += sizeof(D3DXVECTOR3);	//座標分進める.
		pVertices += sizeof(D3DXVECTOR3);	//法線分進める.
		pVertices += sizeof(D3DXVECTOR2);	//法線分進める.
		pUV = (D3DXVECTOR3*)pVertices;
		pVB->Unlock();
	}
	SAFE_RELEASE(pVB);
...
D3DXVECTOR3 D3DXPARSER::GetBinormal(MYMESHCONTAINER* pContainer, DWORD Index)
{
	LPDIRECT3DVERTEXBUFFER9 pVB = nullptr;
	pContainer->MeshData.pMesh->GetVertexBuffer(&pVB);
	DWORD Stride = pContainer->MeshData.pMesh->GetNumBytesPerVertex();
